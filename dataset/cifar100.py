...
def get_cifar100_subset_dataloaders(batch_size=128, num_workers=8, is_instance=False):
    """
    CIFAR-100 Subset Loader: Selects the first 20 classes
    """
    data_folder = get_data_folder()
    ...
    def filter_classes(dataset, classes_to_include):
        indices = [i for i, label in enumerate(dataset.targets) if label in classes_to_include]
        dataset.targets = [dataset.targets[i] for i in indices]
        dataset.data = dataset.data[indices]
        return dataset

    classes_to_include = list(range(20))  # Select classes 0 to 19
    ...
    if is_instance:
        train_set = CIFAR100Instance(root=data_folder,
                                     ...
                                     train=True,
                                     transform=train_transform)
        train_set = filter_classes(train_set, classes_to_include)
        n_data = len(train_set)
    else:
        train_set = datasets.CIFAR100(root=data_folder,
                                      download=True,
    ...
    test_set = datasets.CIFAR100(root=data_folder,
                                 ...
                                 train=False,
                                 transform=test_transform)
    test_set = filter_classes(test_set, classes_to_include)
