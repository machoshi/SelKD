...
from dataset.cifar100 import get_cifar100_dataloaders, get_cifar100_dataloaders_sample, get_cifar100_subset_dataloaders
...
from helper.loops import train_distill as train, validate, train_distill_close as train_close, train_distill_open as train_open, validate_open
...
def main():
    ...
    if opt.dataset == 'cifar100':
        if opt.distill in ['crd']:
            ...
            train_loader, val_loader, n_data = get_cifar100_dataloaders(batch_size=opt.batch_size,
                                                                        num_workers=opt.num_workers,
                                                                        is_instance=True)
            # To change to closed set, get_cifar100_dataloaders -> get_cifar100_subset_dataloaders as in the paper
        n_cls = 20
    else:
        raise NotImplementedError(opt.dataset)
    # model
    model_t = load_teacher(opt.path_t, 100)
    model_s = model_dict[opt.model_s](num_classes=21)
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        time1 = time.time()
        train_acc, train_loss = train_open(epoch, train_loader, module_list, criterion_list, optimizer, opt)
        time2 = time.time()
        ...
        test_acc, tect_acc_top5, test_loss = validate_open(val_loader, model_s, criterion_cls, opt)
